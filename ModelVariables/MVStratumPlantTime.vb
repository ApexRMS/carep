...
'************************************************************************************
''' <summary>
''' ModelVariable3 of type Double with dimension of stratumID, plantID and daily timestep
''' </summary>
''' <remarks></remarks>
Class MVStratumPlantTime
    ...
    End Property
    ''' <summary>
    ''' Value for the specified stratumID, plantID and timestep
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Default Public Overloads Property Item(ByVal stratumID As Integer, ByVal plantID As Integer, ByVal timestep As Integer) As Double
        Get
            Return MyBase.Item(stratumID, plantID, timestep)
        End Get
        Set(ByVal value As Double)
            ValidateAssignment(value)
            MyBase.Item(stratumID, plantID, timestep) = value
        End Set
    End Property
    ''' <summary>
    ...
    End Sub
    ''' <summary>
    ''' Completes instantiation as a thre-dimensional array, with maximum stratumID, plantID and timestep specified as Integers
    ''' </summary>
    ''' <remarks></remarks>
    Public Overrides Sub CreateDimensions(ByVal maxStratumID As Integer, ByVal maxPlantID As Integer, ByVal maxTimestep As Integer)
        Dim maxIndexes() As Integer = {maxStratumID, maxPlantID, maxTimestep}
    ...
    Public Overrides Sub DataSource(ByVal sourceYearJDayDataTable As DataTable, ByVal filterFieldNames() As String)
        ...
        m_mvSource.CreateDimensions(MVStratumPlantTime.DefaultDimensionsSource)  'assign default dimensions for shadow source
    ...
    Public Overrides Sub Load(ByVal sourceFilterValues As Integer())
        ...
        sourceDataView.RowFilter = String.Format("(Iteration={0} OR Iteration IS NULL) AND (Timestep <= {1} OR Timestep IS NULL)", sourceFilterValues(0), MVStratumPlantTime.m_EndYear)
        ...
        If sourceDataView.Count > 0 Then
            ...
            For Each dr As DataRow In Me.SourceDataTableFiltered.Rows
                ...
                Dim yr As Integer = MVStratumPlantTime.m_StartYear
                    ...
                    For Each y As Integer In SLYList
                        For Each z As Integer In SLZList
                            ...
                            If (IterationNull) Then
                                ...
                                If (Me.HasValue(si, PlantIndex, timestep)) Then
                                    Continue For
                                End If
                            ...
                            If (Me.HasValue(si, PlantIndex, timestep)) Then
                                ...
                                Dim msg As String = String.Format(
                                    ...
                                    "Property: {0}Vegetation Type: {1}Climate Zone: {2}Development: {3}Plant: {4}Year: {5}",
                                    ...
                                    m_StratumMultiKeyHelper.Project.GetDataSheet("CAREP_Plant").ValidationTable.GetDisplayName(PlantId) & vbCrLf,
                                    timestep)
                            ...
                            Me.FillItem(si, PlantIndex, timestep) = CDbl(dr(valCol))
    ...
    ''' <summary>
    ''' Fills and interpolates from an existing source Double model variable with dimensions stratum, plant, year, julian day
    ''' </summary>
    ''' <param name="mvStratumPlantYearJDay">source ModelVariable4 with dimensions stratum, plant, year, julian day (in order)</param>
    ...
    ''' <remarks>Assumes that the dimensions of the source model variable are in order of stratum, plant, year, julian day</remarks>
    Public Sub FillFromYearJDay(ByVal mvStratumPlantYearJDay As ModelVariable4(Of Double), ByVal startYear As Integer, ByVal startJDay As Integer)
        ...
        minYr = mvStratumPlantYearJDay.Dimensions(2).MinIndex
        maxYr = mvStratumPlantYearJDay.Dimensions(2).MaxIndex
        minJday = mvStratumPlantYearJDay.Dimensions(3).MinIndex
        maxJDay = mvStratumPlantYearJDay.Dimensions(3).MaxIndex

        m_SourceFieldName = mvStratumPlantYearJDay.SourceFieldName   ' pass the source field name to the new ModelVariable
                ...
                For yr = minYr To maxYr
                    For jDay = minJday To maxJDay
                        ...
                        If timestep >= 0 And timestep <= maxTimestep Then
                            If mvStratumPlantYearJDay.FromSource(s, p, yr, jDay) Then
                                'source element was filled from a data source
                                Me.FillItem(s, p, timestep) = mvStratumPlantYearJDay(s, p, yr, jDay)
                            End If
                        End If
                    Next
                Next
